...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    //
    {
        // Stelnet debug (#83 r9 follow-up #5): force a memory barrier
        // before every op to test if any unrecognised concurrency hazard
        // is the cause of Bug B. STELNET_METAL_FORCE_BARRIER=1.
        static const char * dbg_barrier = std::getenv("STELNET_METAL_FORCE_BARRIER");
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    // Stelnet patch (#83): when an op carries the GGML_PREC_F32 hint, route
    ...
    // Stelnet patch (#83): the deepest fix path — when PREC_F32 is set with
    ...
    // Stelnet patch (#83 r9): Q8_0 weights × F32 input with PREC_F32 —
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        (
         ...
         // Stelnet patch (#83): force mul_mv_ext for PREC_F32 even at
        ...
        if (prec_f32 && ne11 > 8) {
            // Stelnet patch (#83): for PREC_F32 with ne11 > 8, pick a fixed
...
// Stelnet patch (#83): extra Q8_K-quantised input buffer for the
...
// Stelnet patch (#83 r9): extra Q8_0-quantized input buffer for the
...
int ggml_metal_op_im2col(ggml_metal_op_t ctx, int idx) {
    ...
    // Stelnet debug (#83 r9 follow-up #5): host-side readback of op->src[1]
    ...
    {
        static const char * dbg_env = std::getenv("STELNET_IM2COL_DBG");
        static int dbg_n = 0;
...
// Stelnet patch (PR #160 col2im_1d): dispatch the col2im_1d gather kernel.
...
// Stelnet patch (PR #07-metal-aa-snake-beta): dispatch the fused
