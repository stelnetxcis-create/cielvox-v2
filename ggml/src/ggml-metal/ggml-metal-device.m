...
// Stelnet patch (PLAN #88 / CrisperWeaver §5.18): forward
...
static id<MTLBinaryArchive> stelnet_metal_device_archive(ggml_metal_device_t dev);
static void stelnet_metal_device_mark_archive_dirty(ggml_metal_device_t dev);
    ...
    // TODO: move to a function
    {
        ...
        if (!library) {
            @autoreleasepool {
                ...
                // Stelnet patch (#83): disable fast-math when STELNET_METAL_STRICT_FP=1.
                ...
                // of some throughput.
                if (getenv("STELNET_METAL_STRICT_FP")) {
...
struct ggml_metal_pipeline_with_params ggml_metal_library_compile_pipeline(ggml_metal_library_t lib, const char * base, const char * name, ggml_metal_cv_t cv) {
    ...
    @autoreleasepool {
        ...
        // Stelnet patch (PLAN #88 / CrisperWeaver §5.18): pipeline
        ...
        id<MTLBinaryArchive> archive = stelnet_metal_device_archive(lib->dev);
        if (archive) {
        ...
        if (obj && archive) {
            ...
            if ([archive addComputePipelineFunctionsWithDescriptor:pdesc error:&add_err]) {
                stelnet_metal_device_mark_archive_dirty(lib->dev);
            } else if (add_err) {
...
struct ggml_metal_device {
    ...
    // Stelnet patch (PLAN #88 / CrisperWeaver §5.18): persistent
    ...
    // Stelnet / CrisperWeaver process spawn. The archive lets us
...
// ───────────────────────────────────────────────────────────────────
// Stelnet patch (PLAN #88 / CrisperWeaver §5.18): MTLBinaryArchive
...
static NSURL * stelnet_metal_pipeline_cache_url(NSString * device_name) {
...
static void stelnet_metal_pipeline_cache_open(ggml_metal_device_t dev) {
    ...
    NSURL * url = stelnet_metal_pipeline_cache_url([dev->mtl_device name]);
    if (!url) {
        return;
    }
...
static void stelnet_metal_pipeline_cache_flush(ggml_metal_device_t dev) {
...
ggml_metal_device_t ggml_metal_device_init(int device) {
    ...
    if (dev->mtl_device == nil) {
        ...
        if (dev->mtl_device) {
            ...
            // Stelnet patch (PLAN #88 / CrisperWeaver §5.18): open the
            ...
            stelnet_metal_pipeline_cache_open(dev);
...
void ggml_metal_device_free(ggml_metal_device_t dev) {
    ...
    // Stelnet patch (PLAN #88 / CrisperWeaver §5.18): flush the
    ...
    stelnet_metal_pipeline_cache_flush(dev);
...
// Stelnet patch (PLAN #88 / CrisperWeaver §5.18): the pipeline-
...
bool ggml_metal_device_supports_op(ggml_metal_device_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_CONV_TRANSPOSE_1D:
            return true;
        // Stelnet patch (PR #160 col2im_1d) — MUST RE-APPLY after ggml bump.
        case GGML_OP_COL2IM_1D:
            return op->src[0]->type == GGML_TYPE_F32 || op->src[0]->type == GGML_TYPE_F16;
        // Stelnet patch (PR #07-metal-aa-snake-beta) — MUST RE-APPLY after ggml bump.
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            // Stelnet patch (#83): if the op carries PREC_F32, refuse to
...
void ggml_metal_buffer_set_tensor(ggml_metal_buffer_t buf, struct ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    if (buf->is_shared && !getenv("STELNET_FORCE_BLIT_COPY")) {
        memcpy((char *) tensor->data + offset, data, size);
        // Stelnet debug (#83 r9 follow-up #5): probe CPU/GPU cache coherency
        // hypothesis. STELNET_FORCE_DMB=1 inserts a full memory barrier
        // after the host memcpy so the GPU is guaranteed to observe the
        // writes when the subsequent Metal command buffer executes.
        if (getenv("STELNET_FORCE_DMB")) {
            __sync_synchronize();
        }
        return;
    }
    // Stelnet debug (#83 r9 follow-up #5): if STELNET_FORCE_BLIT_COPY=1,
