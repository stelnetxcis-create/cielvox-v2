...
void dequantize_q4_K(device const block_q4_K * xb, short il, thread type4x4 & reg) {
    ...
    // Stelnet patch (#83): mirror the CPU `dequantize_row_q4_K` arithmetic
...
// Stelnet patch — explicit per-T reduction helpers for kernel_norm_fuse_impl
...
static inline float stelnet_vec_sum(float v)  { return v; }
static inline float stelnet_vec_sum(float4 v) { return v.x + v.y + v.z + v.w; }
static inline float stelnet_vec_sqsum(float v)  { return v * v; }
static inline float stelnet_vec_sqsum(float4 v) { return v.x*v.x + v.y*v.y + v.z*v.z + v.w*v.w; }
...
kernel void kernel_norm_fuse_impl(
    ...
    sumf = stelnet_vec_sum(sumft); // Stelnet patch — see header
    sumf = simd_sum(sumf);
    ...
    // Stelnet patch — serial reduction by thread 0 of sg0 instead of
    ...
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        y[i00] = x[i00] - mean;
        sumf += stelnet_vec_sqsum(y[i00]); // Stelnet patch — see header
    }
    sumf = simd_sum(sumf);
    ...
    // Stelnet patch — serial reduction (see kernel_norm mean step).
    if (sgitg == 0 && tiisg == 0) {
        const uint n_sg = (ntg.x + 31) / 32;
        float total = 0.0f;
        for (uint sg = 0; sg < n_sg; sg++) {
...
kernel void kernel_rms_norm_fuse_impl(
    ...
    // parallel sum
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        sumf += stelnet_vec_sqsum(x[i00]); // Stelnet patch — see kernel_norm header
    }
    sumf = simd_sum(sumf);
    ...
    // Stelnet patch — serial reduction by thread 0 (see kernel_norm header).
    if (sgitg == 0 && tiisg == 0) {
        const uint n_sg = (ntg.x + 31) / 32;
        float total = 0.0f;
        for (uint sg = 0; sg < n_sg; sg++) {
...
kernel void kernel_conv_transpose_1d(
    ...
    // Stelnet patch — restrict the input-position loop to the range
...
// ── Stelnet patch (PR #160 col2im_1d decomposition) ──────────────────
...
// ── Stelnet patch (PR #07-metal-aa-snake-beta) ─────────────────────
...
// ── Stelnet patch (PR #07-metal-aa-snake-beta) — opt-in variants ───
...
// Stelnet patch (#83): Q8_K input quantize kernel.
//
...
// Stelnet patch (#83): Q4_K × Q8_K mat-vec multiply, F32 output.
//
...
// Stelnet patch (#83 r9): F32 → Q8_0 input quantize, mirrors CPU's
...
// Stelnet patch (#83 r9): Q8_0 × Q8_0 mat-vec multiply, F32 output.
//
...
// Stelnet patch (#83): high-precision mul_mm variant — keeps F32 inputs in
...
// Stelnet patch (#83): high-precision (F32-tile) mul_mm instantiations.
