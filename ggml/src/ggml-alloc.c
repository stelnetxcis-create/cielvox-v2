...
// Per-node allocator trace. Gated by STELNET_GGML_ALLOC_TRACE=1. Emits one
// line per allocate/free decision in a diffable format. Use
// STELNET_GGML_ALLOC_TRACE_MAX_PASSES=N to cap (sched runs the allocator
...
static bool ggml_alloc_trace_init_env(void) {
    if (s_alloc_trace_max_passes == -1) {
        const char * env = getenv("STELNET_GGML_ALLOC_TRACE");
        if (env && *env) {
            const char * max_env = getenv("STELNET_GGML_ALLOC_TRACE_MAX_PASSES");
