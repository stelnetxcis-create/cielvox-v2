...
- 38 have a `stelnet-diff` C++ harness branch
...
`examples/cli/stelnet_diff_main.cpp`. Then `tools/bootstrap_ref_env.sh
...
stelnet-diff chatterbox <t3.gguf> <ref.gguf> <any.wav>`. The Arabic ref is
...
These are wired into `stelnet-diff` but skip every stage at runtime
