    ...
    def do_GET(self):
        ...
        if self.path.startswith(CONTEXT_ROOT):
            ...
            if '.worker.js' in actual_path:
                ...
                self.path = '/stelnet.wasm/index.html'
            ...
            # For all other paths under the context root
                ...
                # Check if this is a request to a file in stelnet.wasm
                potential_file = os.path.join(DIRECTORY, 'stelnet.wasm', actual_path.lstrip('/'))
                if os.path.exists(potential_file) and not os.path.isdir(potential_file):
                    self.path = '/stelnet.wasm' + actual_path
                else:
