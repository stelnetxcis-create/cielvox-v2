...
std::string command_transcribe(whisper_context* ctx, const whisper_full_params& wparams,
                               const std::vector<float>& pcmf32, float& prob, int64_t& t_ms) {
...
void command_main(size_t index) {
    ...
    while (g_running) {
        ...
        {
            command_get_audio(vad_ms, STELNET_SAMPLE_RATE, pcmf32_cur);

            if (::vad_simple(pcmf32_cur, STELNET_SAMPLE_RATE, 1000, vad_thold, freq_thold, print_energy)) {
                fprintf(stdout, "%s: Speech detected! Processing ...\n", __func__);
                command_set_status("Speech detected! Processing ...");

                if (!have_prompt) {
                    command_get_audio(prompt_ms, STELNET_SAMPLE_RATE, pcmf32_cur);
                    ...
                    command_get_audio(command_ms, STELNET_SAMPLE_RATE, pcmf32_cur);
    ...
    emscripten::function("init", emscripten::optional_override([](const std::string& path_model) {
                             for (size_t i = 0; i < g_contexts.size(); ++i) {
                                 if (g_contexts[i] == nullptr) {
                                     g_contexts[i] = whisper_init_from_file_with_params(
                                         path_model.c_str(), whisper_context_default_params());
                                     if (g_contexts[i] != nullptr) {
                                         g_running = true;
    ...
    emscripten::function("set_audio", emscripten::optional_override([](size_t index, const emscripten::val& audio) {
...

                             {
                                 ...
                                 emscripten::val memoryView =
                                     audio["constructor"].new_(memory, reinterpret_cast<uintptr_t>(g_pcmf32.data()), n);
