...
void stream_main(size_t index, const std::string& lang) {
    ...
    // 5 seconds interval
    const int64_t window_samples = 5 * STELNET_SAMPLE_RATE;
    ...
    while (g_running) {
...

        {
            ...
            pcmf32 =
                std::vector<float>(g_pcmf32.end() - std::min((int64_t)g_pcmf32.size(), window_samples), g_pcmf32.end());
            g_pcmf32.clear();
        }
...
EMSCRIPTEN_BINDINGS(stream) {
    emscripten::function(
        "init", emscripten::optional_override([](const std::string& path_model, const std::string& lang) {
            for (size_t i = 0; i < g_contexts.size(); ++i) {
                if (g_contexts[i] == nullptr) {
                    g_contexts[i] =
                        whisper_init_from_file_with_params(path_model.c_str(), whisper_context_default_params());
                    if (g_contexts[i] != nullptr) {
                        g_running = true;
                        if (g_worker.joinable()) {
                            g_worker.join();
                        }
                        g_worker = std::thread([i, lang]() { stream_main(i, lang); });
