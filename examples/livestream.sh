#!/bin/bash
#
# Transcribe audio livestream by feeding ffmpeg output to stelnet at regular intervals
...
ffmpeg -loglevel quiet -y -re -probesize 32 -i $url -c copy /tmp/stelnet-live0.${fmt} &
if [ $? -ne 0 ]; then
...
while [ $running -eq 1 ]; do
    ...
    while [ $err -ne 0 ]; do
        if [ $i -gt 0 ]; then
            ffmpeg -loglevel quiet -v error -noaccurate_seek -i /tmp/stelnet-live0.${fmt} -y -ar 16000 -ac 1 -c:a pcm_s16le -ss $(($i*$step_s-1)).5 -t $step_s /tmp/stelnet-live.wav 2> /tmp/stelnet-live.err
        else
            ffmpeg -loglevel quiet -v error -noaccurate_seek -i /tmp/stelnet-live0.${fmt} -y -ar 16000 -ac 1 -c:a pcm_s16le -ss $(($i*$step_s)) -t $step_s /tmp/stelnet-live.wav 2> /tmp/stelnet-live.err
        fi
        err=$(cat /tmp/stelnet-live.err | wc -l)
    done

    ./build/bin/stelnet -t 8 -m ./models/ggml-${model}.bin -f /tmp/stelnet-live.wav --no-timestamps -otxt 2> /tmp/stelnet.err | tail -n 1
