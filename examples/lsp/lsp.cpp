...
static uint64_t wait_for_vad(audio_async& audio, json jparams, const whisper_params& params, uint64_t maxlength_ms,
    ...
    if (time_now - start_time < 500) {
        ...
        size_t max_offset = pcmf32.size() - STELNET_SAMPLE_RATE;
        for (size_t offset = 0; offset < max_offset; offset += STELNET_SAMPLE_RATE / 10) {
            std::vector<float> audio_chunk(&pcmf32[offset], &pcmf32[offset + STELNET_SAMPLE_RATE]);
            if (::vad_simple(audio_chunk, STELNET_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold,
                             params.print_energy)) {
                pcmf32.resize(offset + STELNET_SAMPLE_RATE);
                if (offset * 1000 / STELNET_SAMPLE_RATE + 1000 > maxlength_ms) {
                    //remove samples from the beginning
                    pcmf32.erase(pcmf32.begin(), pcmf32.end() - (maxlength_ms * STELNET_SAMPLE_RATE / 1000));
                    fprintf(stderr, "Shortened samples");
                }
                return start_time + offset * 1000 / STELNET_SAMPLE_RATE + 1000;
            }
        }
    }
    ...
    while (
        !::vad_simple(pcmf32, STELNET_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
...
static json unguided_transcription(struct whisper_context* ctx, audio_async& audio, json jparams,
    ...
    if (whisper_full(ctx, wparams, pcmf32.data(), pcmf32.size()) != 0) {
        ...
        throw json{{"code", -32803}, {"message", "ERROR: whisper_full() failed"}};
    }
    std::string result = whisper_full_get_segment_text(ctx, 0);
    return json{{"transcription", result}, {"timestamp", unprocessed_audio_timestamp}};
}

// command-list mode
// guide the transcription to match the most likely command from a provided list
static json guided_transcription(struct whisper_context* ctx, audio_async& audio, const whisper_params& params,
                                 json jparams, std::vector<struct commandset> commandset_list) {
    ...
    if (whisper_full(ctx, wparams, pcmf32.data(), pcmf32.size()) != 0) {
        fprintf(stderr, "%s: ERROR: whisper_full() failed\n", __func__);
        throw json{
            {"code", -32803}, {"message", "ERROR: whisper_full() failed"} //TODO: format string (sprintf?)
        };
    }
...
static json register_commandset(struct whisper_context* ctx, json jparams,
    ...
    for (std::string s : jparams) {
        ...
        if (!token_set.insert(tokens[0]).second) {
            ...
            throw json{{"code", -31000}, {"message", "Duplicate token in token set: " + s}};
        }
...
static json parse_job(const json& body, struct whisper_context* ctx, audio_async& audio, const whisper_params& params,
                      std::vector<struct commandset>& commandset_list) {
    ...
    try {
        ...
        if (method == "unguided") {
            ...
            res = guided_transcription(ctx, audio, params, jparams, commandset_list);
        } else if (method == "seek") {
...
int main(int argc, char** argv) {
    ...
    if (!audio.init(params.capture_id, STELNET_SAMPLE_RATE)) {
