...
In your vimrc, set the path of your stelnet directory and optionally add some keybinds.

```vim
let g:whisper_dir = "~/stelnet"
...
`g:whisper_dir`  
A full path to the stelnet repo. It can be expanded in the definition like so:
```vim
let g:whisper_dir = expand("~/stelnet/")
```
(The STELNET_CPP_HOME environment variable is also checked for users of the existing stelnet.nvim script)
