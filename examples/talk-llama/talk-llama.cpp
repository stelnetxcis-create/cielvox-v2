...
static bool whisper_params_parse(int argc, char** argv, whisper_params& params) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            params.min_keep = std::stoul(argv[++i]);
        } else if (arg == "--top-p") {
...
static std::string transcribe(whisper_context* ctx, const whisper_params& params, const std::vector<float>& pcmf32,
                              const std::string prompt_text, float& prob, int64_t& t_ms) {
...
int main(int argc, char** argv) {
    ...
    if (!ctx_wsp) {
        fprintf(stderr, "No stelnet model specified. Please provide using -mw <modelfile>\n");
        return 1;
    }
    ...
    if (session_tokens.size()) {
        ...
        if (n_matching_session_tokens >= embd_inp.size()) {
            ...
            fprintf(stderr,
                    "%s: warning: session file has low similarity to prompt (%zu / %zu tokens); will mostly be "
                    "reevaluated\n",
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, STELNET_SAMPLE_RATE, 1250, params.vad_thold, params.freq_thold,
                             params.print_energy) ||
                force_speak) {
