...
#include "stelnet.h"
#include "stelnet_session.h" // session C-ABI for the backend-agnostic transcribeSession() path
    ...
    int run_with_progress(whisper_params& params, whisper_result& result) {
        ...
        for (int f = 0; f < (int)params.fname_inp.size(); ++f) {
            ...
            // Run inference
            {
                ...
                wparams.strategy = params.beam_size > 1 ? STELNET_SAMPLING_BEAM_SEARCH : STELNET_SAMPLING_GREEDY;
...
// ---------------------------------------------------------------------------
// Session-based transcription via the stelnet_session C-ABI. Unlike whisper()
...
static int run_session(whisper_params& params, whisper_result& result) {
    ...
    stelnet_session* s =
        params.backend.empty()
            ? stelnet_session_open(params.model.c_str(), params.n_threads)
            : stelnet_session_open_explicit(params.model.c_str(), params.backend.c_str(), params.n_threads);
    if (!s) {
        fprintf(stderr, "error: failed to open stelnet session (model '%s', backend '%s')\n", params.model.c_str(),
                params.backend.c_str());
        return 3;
    }

    if (!params.source_lang.empty())
        stelnet_session_set_source_language(s, params.source_lang.c_str());
    if (!params.target_lang.empty())
        stelnet_session_set_target_language(s, params.target_lang.c_str());
    stelnet_session_set_punctuation(s, params.punctuation ? 1 : 0);
    if (!params.punc_model.empty())
        stelnet_session_set_punc_model(s, params.punc_model.c_str());
    stelnet_session_set_translate(s, params.translate ? 1 : 0);
    if (params.beam_size > 0)
        stelnet_session_set_beam_size(s, params.beam_size);
    if (params.temperature > 0.0f)
        stelnet_session_set_temperature(s, params.temperature, params.seed);
    ...
    stelnet_session_result* r = stelnet_session_transcribe_lang(s, pcmf32.data(), (int)pcmf32.size(), lang);
    if (!r) {
        stelnet_session_close(s);
        return 5;
    }
    ...
    const int n = stelnet_session_result_n_segments(r);
    result.segments.resize(n);
    for (int i = 0; i < n; ++i) {
        const int64_t t0 = stelnet_session_result_segment_t0(r, i);
        const int64_t t1 = stelnet_session_result_segment_t1(r, i);
