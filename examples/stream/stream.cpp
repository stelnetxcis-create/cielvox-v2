...
int main(int argc, char** argv) {
    ...
    const int n_samples_step = (1e-3 * params.step_ms) * STELNET_SAMPLE_RATE;
    const int n_samples_len = (1e-3 * params.length_ms) * STELNET_SAMPLE_RATE;
    const int n_samples_keep = (1e-3 * params.keep_ms) * STELNET_SAMPLE_RATE;
    const int n_samples_30s = (1e-3 * 30000.0) * STELNET_SAMPLE_RATE;
    ...
    // print some info about the processing
    {
        ...
        fprintf(stderr,
                ...
                __func__, n_samples_step, float(n_samples_step) / STELNET_SAMPLE_RATE,
                float(n_samples_len) / STELNET_SAMPLE_RATE, float(n_samples_keep) / STELNET_SAMPLE_RATE,
    ...
    if (params.save_audio) {
        ...
        wavWriter.open(filename, STELNET_SAMPLE_RATE, 16, 1);
    }
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            if (::vad_simple(pcmf32_new, STELNET_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, false)) {
        ...
        // run the inference
        {
            whisper_full_params wparams = whisper_full_default_params(
                params.beam_size > 1 ? STELNET_SAMPLING_BEAM_SEARCH : STELNET_SAMPLING_GREEDY);
            ...
            // print result;
            {
                if (!use_vad) {
                    ...
                    const int64_t t0 = std::max(0.0, t1 - pcmf32.size() * 1000.0 / STELNET_SAMPLE_RATE);
