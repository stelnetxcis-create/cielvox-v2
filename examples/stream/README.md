# stelnet/examples/stream

This is a naive example of performing real-time inference on audio from your microphone.
The `stelnet-stream` tool samples the audio every half a second and runs the transcription continously.
More info is available in [issue #10](https://github.com/ggml-org/whisper.cpp/issues/10).

```bash
./build/bin/stelnet-stream -m ./models/ggml-base.en.bin -t 8 --step 500 --length 5000
```
...
```bash
 ./build/bin/stelnet-stream -m ./models/ggml-base.en.bin -t 6 --step 0 --length 30000 -vth 0.6
```
...
The `stelnet-stream` tool depends on SDL2 library to capture audio from the microphone. You can build it like this:
...
cmake -B build -DSTELNET_SDL2=ON
cmake --build build --config Release

./build/bin/stelnet-stream
```
