...
static std::string transcribe(whisper_context* ctx, const whisper_params& params, const std::vector<float>& pcmf32,
                              const std::string& grammar_rule, float& logprob_min, float& logprob_sum, int& n_tokens,
                              int64_t& t_ms) {
    ...
    whisper_full_params wparams = whisper_full_default_params(STELNET_SAMPLING_BEAM_SEARCH);
...
static int process_command_list(struct whisper_context* ctx, audio_async& audio, const whisper_params& params,
    ...
    while (is_running) {
        ...
        if (::vad_simple(pcmf32_cur, STELNET_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold,
                         params.print_energy)) {
            ...
            // NOTE: not optimal
            {
                ...
                // print the commands and the respective probabilities
                {
                    ...
                    for (const auto& cmd : probs_id) {
                        fprintf(stdout, "%s: %s%-*s%s = %f | ", __func__, "\033[1m", max_len,
                                allowed_commands[cmd.second].c_str(), "\033[0m", cmd.first);
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, STELNET_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold,
                ...
                const auto txt =
                    ::trim(::transcribe(ctx, params, pcmf32_cur, "", logprob_min, logprob_sum, n_tokens, t_ms));
...
static int process_general_transcription(struct whisper_context* ctx, audio_async& audio, const whisper_params& params,
    ...
    while (is_running) {
        ...
        if (ask_prompt) {
            fprintf(stdout, "\n");
            fprintf(stdout, "%s: Say the following phrase: '%s%s%s'\n", __func__, "\033[1m", k_prompt.c_str(),
                    "\033[0m");
            fprintf(stdout, "\n");
...

        {
            ...
            if (::vad_simple(pcmf32_cur, STELNET_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold,
                ...
                if (!have_prompt) {
                    ...
                    const auto txt = ::trim(
                        ::transcribe(ctx, params, pcmf32_cur, "prompt", logprob_min0, logprob_sum0, n_tokens0, t_ms));

                    const float p = 100.0f * std::exp(logprob_min0);

                    fprintf(stdout, "%s: Heard '%s%s%s', (t = %d ms, p = %.2f%%)\n", __func__, "\033[1m", txt.c_str(),
                            "\033[0m", (int)t_ms, p);

                    const float sim = similarity(txt, k_prompt);

                    if (txt.length() < 0.8 * k_prompt.length() || txt.length() > 1.2 * k_prompt.length() ||
                        sim < 0.8f) {
                    ...
                    //printf("len prompt:  %.4f\n", pcmf32_prompt.size() / (float) STELNET_SAMPLE_RATE);
                    //printf("len command: %.4f\n", pcmf32_cur.size() / (float) STELNET_SAMPLE_RATE);

                    // prepend 3 second of silence
                    pcmf32_cur.insert(pcmf32_cur.begin(), 3.0f * STELNET_SAMPLE_RATE, 0.0f);

                    // prepend the prompt audio
                    pcmf32_cur.insert(pcmf32_cur.begin(), pcmf32_prompt.begin(), pcmf32_prompt.end());

                    const auto txt =
                        ::trim(::transcribe(ctx, params, pcmf32_cur, "root", logprob_min, logprob_sum, n_tokens, t_ms));
