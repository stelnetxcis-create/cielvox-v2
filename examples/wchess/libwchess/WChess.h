...
public:
    ...
    WChess(whisper_context* ctx, const whisper_full_params& wparams, callbacks cb, settings s);
    ~WChess();
...
private:
    ...
    std::string transcribe(const std::vector<float>& pcmf32, float& logprob_min, float& logprob_sum, int& n_tokens,
                           int64_t& t_ms);
