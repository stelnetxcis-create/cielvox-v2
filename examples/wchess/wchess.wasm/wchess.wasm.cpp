...
void set_move(const std::string& move, float prob) {
    MAIN_THREAD_EM_ASM({setMove(UTF8ToString($0), $1)}, move.c_str(), prob);
}

void set_grammar(const std::string& grammar) {
    MAIN_THREAD_EM_ASM({setGrammar(UTF8ToString($0))}, grammar.c_str());
}
...
void wchess_main(size_t i) {
    ...
    wparams.initial_prompt = "bishop to c3, rook to d4, knight to e5, d4 d5, knight to c3, c3, queen to d4, king b1, "
                             ...
                             for (size_t i = 0; i < g_contexts.size(); ++i) {
                                 if (g_contexts[i] == nullptr) {
                                     ...
                                     if (g_contexts[i] != nullptr) {
                                         ...
                                         g_worker = std::thread([i]() { wchess_main(i); });
