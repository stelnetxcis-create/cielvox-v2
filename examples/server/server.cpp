...
int main(int argc, char** argv) {
    ...
    svr->set_default_headers({{"Server", "stelnet"},
    ...
    svr->Post(sparams.request_path + sparams.inference_path, [&](const Request& req, Response& res) {
        ...
        if (sparams.ffmpeg_converter) {
            ...
            const std::string temp_filename = generate_temp_filename(sparams.tmp_dir, "stelnet-server", ".wav");
        ...
        // print some info about the processing
        {
            ...
            fprintf(stderr,
                    ...
                    "%stimestamps = %d ...\n",
                    __func__, filename.c_str(), int(pcmf32.size()), float(pcmf32.size()) / STELNET_SAMPLE_RATE,
        ...
        // run the inference
        {
            printf("Running stelnet inference on %s\n", filename.c_str());
        ...
        if (params.response_format == text_format) {
            ...
            json jres = json{{"task", params.translate ? "translate" : "transcribe"},
                             ...
                             {"duration", float(pcmf32.size()) / STELNET_SAMPLE_RATE},
                             {"text", results},
                             {"segments", json::array()}};
