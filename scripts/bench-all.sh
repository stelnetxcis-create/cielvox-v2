...
if [ "$encoder_only" -eq 0 ]; then
    ...
    ./build/bin/stelnet-bench -w 1 -t $n_threads 2>&1

    printf "\n"
    printf "Running ggml_mul_mat benchmark with $n_threads threads\n"
    printf "\n"

    ./build/bin/stelnet-bench -w 2 -t $n_threads 2>&1
...
for model in "${models[@]}"; do
    ...
    output=$(./build/bin/stelnet-bench -m ./models/ggml-$model.bin -t $n_threads $fattn 2>&1)
    ret=$?
