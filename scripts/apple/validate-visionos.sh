#!/bin/bash
# validate-visionos.sh - Validate visionOS Application with embedded stelnet.xcframework using SwiftUI
...
XCFRAMEWORK_PATH="${ROOT_DIR}/build-apple/stelnet.xcframework"
...
/* Begin PBXBuildFile section */
        ...
        55555555555555555555555 /* stelnet.xcframework in Frameworks */ = {isa = PBXBuildFile; fileRef = 66666666666666666666666; };
        77777777777777777777777 /* stelnet.xcframework in Embed Frameworks */ = {isa = PBXBuildFile; fileRef = 66666666666666666666666; };
...
/* Begin PBXCopyFilesBuildPhase section */
        88888888888888888888888 /* Embed Frameworks */ = {
            ...
            dstSubfolderSpec = 10;
            files = (
                77777777777777777777777 /* stelnet.xcframework in Embed Frameworks */,
            );
...
cat >> "${TEMP_DIR}/${APP_NAME}/${APP_NAME}.xcodeproj/project.pbxproj" << EOF
        ...
        66666666666666666666666 /* stelnet.xcframework */ = {isa = PBXFileReference; lastKnownFileType = wrapper.xcframework; path = stelnet.xcframework; sourceTree = "<group>"; };
...
/* Begin PBXFrameworksBuildPhase section */
        BBBBBBBBBBBBBBBBBBBBBBBB /* Frameworks */ = {
            ...
            buildActionMask = 2147483647;
            files = (
                55555555555555555555555 /* stelnet.xcframework in Frameworks */,
            );
            runOnlyForDeploymentPostprocessing = 0;
        };
...
cat >> "${TEMP_DIR}/${APP_NAME}/${APP_NAME}.xcodeproj/project.pbxproj" << 'EOF'
        DDDDDDDDDDDDDDDDDDDDDDDD /* Frameworks */ = {
            isa = PBXGroup;
            children = (
                66666666666666666666666 /* stelnet.xcframework */,
            );
            name = Frameworks;
            sourceTree = "<group>";
        };
...
if grep -q "No suitable application records were found" "${VALIDATION_OUTPUT}"; then
    ...
    if [ -d "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/stelnet.framework" ]; then
        echo "✅ stelnet.framework properly embedded"
    else
        echo "❌ stelnet.framework not properly embedded"
        FINAL_VALIDATION_RESULT=1
    fi

    # Check if framework binary exists
    if [ -f "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/stelnet.framework/stelnet" ]; then
        echo "✅ Framework binary exists"

        # Further validate framework by checking architecture
        ARCHS=$(lipo -info "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/stelnet.framework/stelnet" 2>/dev/null | grep -o "arm64\\|x86_64" | tr '\n' ' ')
        if [ -n "$ARCHS" ]; then
