#!/bin/bash
# validate-ios.sh - Validate iOS Application with embedded stelnet.xcframework using SwiftUI
...
if grep -q "No suitable application records were found" "${VALIDATION_OUTPUT}"; then
    ...
    if [ -f "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/stelnet.framework/stelnet" ]; then
        ...
        ARCHS=$(lipo -info "${TEMP_DIR}/Payload/${APP_NAME}.app/Frameworks/stelnet.framework/stelnet" 2>/dev/null | grep -o "arm64\\|armv7\\|x86_64" | tr '\n' ' ')
        if [ -n "$ARCHS" ]; then
