#!/bin/bash
# validate-macos.sh - Validate macOS Application with embedded stelnet.xcframework using SwiftUI
...
if [ -d "${APP_PATH}/Contents/Frameworks/stelnet.framework" ]; then
...
if [ -f "${APP_PATH}/Contents/Frameworks/stelnet.framework/Versions/A/stelnet" ]; then
    echo "✅ Framework binary exists"

    # Further validate framework by checking architecture
    ARCHS=$(lipo -info "${APP_PATH}/Contents/Frameworks/stelnet.framework/Versions/A/stelnet" 2>/dev/null | grep -o "arm64\\|x86_64" | tr '\n' ' ')
    if [ -n "$ARCHS" ]; then
