class Options
  ...
  def cmake_options
    ...
    Dir.chdir __dir__ do
      output = `#{@cmake.shellescape} -S sources -B build -D STELNET_BUILD_TESTS=OFF -D STELNET_BUILD_EXAMPLES=OFF -D STELNET_BUILD_SERVER=OFF -L`
    end
  ...
  def configure_coreml
    if enabled?("STELNET_COREML")
      $LDFLAGS << " -framework Foundation -framework CoreML"
      $defs << "-DRUBY_STELNET_USE_COREML"
    end
  end
