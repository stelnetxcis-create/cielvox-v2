...
CMAKE_EXTRA="-DSTELNET_FATAL_WARNINGS=ON"
...
function gg_run_bench {
    ...
    if [ "$BENCH_ENCODER_ONLY" -eq 0 ]; then
        ...
        (time ./build-ci-release/bin/stelnet-bench -w 1 -t $BENCH_N_THREADS 2>&1) | tee -a $OUT/${ci}-memcpy.log
        ...
        (time ./build-ci-release/bin/stelnet-bench -w 2 -t $BENCH_N_THREADS 2>&1) | tee -a $OUT/${ci}-mul_mat.log
    ...
    for model in "${MODELS[@]}"; do
        ...
        output=$(./build-ci-release/bin/stelnet-bench -m $MNT/models/ggml-$model.bin -t $BENCH_N_THREADS $fattn 2>&1)
        ret=$?
