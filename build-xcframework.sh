...
STELNET_BUILD_EXAMPLES=OFF
STELNET_BUILD_TESTS=OFF
STELNET_BUILD_SERVER=OFF
GGML_METAL=ON
...
COMMON_CMAKE_ARGS=(
    ...
    -DSTELNET_BUILD_EXAMPLES=${STELNET_BUILD_EXAMPLES}
    -DSTELNET_BUILD_TESTS=${STELNET_BUILD_TESTS}
    -DSTELNET_BUILD_SERVER=${STELNET_BUILD_SERVER}
...
setup_framework_structure() {
    ...
    cp include/stelnet.h          ${header_path}
...
<dict>
    ...
    <string>stelnet</string>
    <key>CFBundleIdentifier</key>
    <string>org.ggml.stelnet</string>
...
combine_static_libraries() {
    ...
    # Auto-discover every static library produced under src/. Stelnet
    ...
    # that stelnet.a publicly depends on via target_link_libraries
    ...
    # (e.g. wav2vec2_load referenced from stelnet_c_api.cpp).
    #
    ...
    while IFS= read -r -d '' lib; do
        src_libs+=("$lib")
    # stelnet_lid / stelnet_punc / stelnet_truecase are STATIC libs added via
    # add_subdirectory(.. ${CMAKE_BINARY_DIR}/stelnet_<x>), so their .a lands
    # under build/stelnet_<x>/, NOT build/src/ — include those roots too or the
    ...
    done < <(find "${base_dir}/${build_dir}/src" "${base_dir}/${build_dir}/stelnet_audio" \
                  "${base_dir}/${build_dir}/stelnet_lid" "${base_dir}/${build_dir}/stelnet_punc" \
                  "${base_dir}/${build_dir}/stelnet_truecase" \
                  -path "*/${release_dir}/*.a" -print0 2>/dev/null)
    if [ ${#src_libs[@]} -gt 0 ]; then
        echo "Auto-discovered ${#src_libs[@]} static backend libraries under src/${release_dir}/ + stelnet_audio/${release_dir}/"
        libs+=("${src_libs[@]}")
    else
        # Fall back to the original explicit list if glob finds nothing
        # (shouldn't happen — defensive).
        echo "warning: glob found no static libs under src/${release_dir}/; falling back to libstelnet.a only"
        libs+=("${base_dir}/${build_dir}/src/${release_dir}/libstelnet.a")
    fi
    # Coreml lib lives under src/ as libstelnet.coreml.a; the glob
    ...
    case "$platform" in
        "ios")
            ...
            install_name="@rpath/stelnet.framework/stelnet"
        ...
        "macos")
            ...
            install_name="@rpath/stelnet.framework/Versions/Current/stelnet"
            frameworks+=" -framework CoreML"
            ;;
        "visionos")
            if [[ "$is_simulator" == "true" ]]; then
    ...
    if [[ "$platform" == "ios" || "$platform" == "visionos" || "$platform" == "tvos" ]]; then
        ...
        xcrun dsymutil "${base_dir}/${output_lib}" -o "${base_dir}/${build_dir}/dSYMs/stelnet.dSYM"
...
cmake -B build-ios-sim -G Xcode \
    ...
    -DSTELNET_COREML="ON" \
    -DSTELNET_COREML_ALLOW_FALLBACK="ON" \
    -S .
...
if [[ "${BUILD_STATIC_XCFRAMEWORK}" == "ON" ]]; then
    xcodebuild -create-xcframework \
        -framework $(pwd)/build-ios-sim/framework/stelnet.framework \
        -framework $(pwd)/build-ios-device/framework/stelnet.framework \
        -framework $(pwd)/build-macos/framework/stelnet.framework \
        -framework $(pwd)/build-visionos/framework/stelnet.framework \
        -framework $(pwd)/build-visionos-sim/framework/stelnet.framework \
        -framework $(pwd)/build-tvos-device/framework/stelnet.framework \
        -framework $(pwd)/build-tvos-sim/framework/stelnet.framework \
        -output $(pwd)/build-apple/stelnet.xcframework
    exit 0
fi

xcodebuild -create-xcframework \
    -framework $(pwd)/build-ios-sim/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-ios-sim/dSYMs/stelnet.dSYM \
    -framework $(pwd)/build-ios-device/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-ios-device/dSYMs/stelnet.dSYM \
    -framework $(pwd)/build-macos/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-macos/dSYMs/stelnet.dSYM \
    -framework $(pwd)/build-visionos/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-visionos/dSYMs/stelnet.dSYM \
    -framework $(pwd)/build-visionos-sim/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-visionos-sim/dSYMs/stelnet.dSYM \
    -framework $(pwd)/build-tvos-device/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-tvos-device/dSYMs/stelnet.dSYM \
    -framework $(pwd)/build-tvos-sim/framework/stelnet.framework \
    -debug-symbols $(pwd)/build-tvos-sim/dSYMs/stelnet.dSYM \
    -output $(pwd)/build-apple/stelnet.xcframework
