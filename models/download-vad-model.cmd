...
rem Check if 'stelnet' is available in the system PATH
where stelnet >nul 2>&1
if %ERRORLEVEL%==0 (
  rem If found, suggest 'stelnet' (relying on PATH resolution)
  set "whisper_cmd=stelnet"
) else (
  rem If not found, suggest the local build version
  set "whisper_cmd=%root_path%\build\bin\Release\stelnet.exe"
)
